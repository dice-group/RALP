...
class MultiLabelLinkPredictionWithScores(dspy.Signature):
    ...
    predicate:str = dspy.InputField(desc="The predicate.")
...
class MultiLabelLinkPredictor(dspy.Module):
    ...
    def _sample_examples_m2(self, subject, predicate, few_shot_examples):
        sampled_few_shot_examples = dict()
        items_for_p = {key: value for key, value in few_shot_examples.items() if key[1] == predicate}
        sampled_few_shot_examples.update(items_for_p)
        return sampled_few_shot_examples
