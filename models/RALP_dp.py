...
class LiteralLinkPrediction(dspy.Signature):
    context:str = dspy.InputField(desc="The context of the subject entity")
    subject:str = dspy.InputField(desc="The subject entity.")
    predicate:str = dspy.InputField(desc="The dataproperty type.")

    predicted_literal = dspy.OutputField(desc="The literal value (number, boolean, or date) that best fits the given subject and predicate.")

class LiteralLinkPredictor(dspy.Module):
    def __init__(self):
        super().__init__()
        self.predictor = dspy.ChainOfThought(LiteralLinkPrediction)
        
    def forward(self, subject, predicate, entity_to_triples, relation_to_triples) -> str:
        context_str = f"Given this context about the subject entity: {subject} and the predicate: {predicate}, predict the literal value that best fits the given subject and predicate.\n\n"
        context_str += f"Context for the predicate:\n"

        for s, p, o in relation_to_triples.get(predicate, []):
            context_str += f"Subject: {s}, Predicate: {p}, Object: {o}\n"
        context_str += f"Context for the subject:\n"
        for s, p, o in entity_to_triples.get(subject, []):
            context_str += f"Subject: {s}, Predicate: {p}, Object: {o}\n"

        context_str += f"The literal value that best fits the given subject and predicate is: "

        context_str += "Rules:"
        context_str += "1. The literal value should be a string, number, boolean, or date.\n"
        context_str += "2. The value should be a valid value for the predicate.\n"
        context_str += "3. The value should be sensible for the given context.\n"
        context_str += "4. Use knowledge of real-world relationships when appropriate.\n\n"

        print(f"PROMPT:\n{context_str}\n")
        dspy_pred = self.predictor(context=context_str, subject=subject, predicate=predicate)
        return dspy_pred.predicted_literal
...
class RALP(AbstractBaseLinkPredictorClass):
    ...
    def __init__(self, knowledge_graph, base_url, api_key, temperature, seed, llm_model, use_val: bool = False):
        ...
        self.entity_to_triples = dict()
        for s, p, o in self.triples:
            self.entity_to_triples.setdefault(s, []).append((s, p, o))

        self.relation_to_triples = dict()
        for s, p, o in self.triples:
    ...
    def _create_literal_predictor(self):
        """Create a literal-based predictor"""
        predictors = []
        for i in self.ensemble:
            dspy.configure(lm=i)
            predictors.append(LiteralLinkPredictor())
        return predictors
            ...
            if r in self.datatype_properties:
                ...
                for predictor in self.literal_predictors:
                    dspy_pred = predictor.forward(
                        subject=h,
                        predicate=r,
                        entity_to_triples=self.entity_to_triples,
                        relation_to_triples=self.relation_to_triples
                    )
                    if dspy_pred is not None:
                        literal_predictions.append(dspy_pred)

                if literal_predictions:
                    most_common_literal, _ =  Counter(literal_predictions).most_common(1)[0]
                    print(f"For {h} and {r}, the most common literal is {most_common_literal} from all literals: {literal_predictions}")
                else:
                    most_common_literal = None
                
                dummy_scores = [0.0] * len(self.idx_to_entity)
                process(dummy_scores, storage, h=h, r=r, literal=most_common_literal)
            else:
    ...
    def get_predicted_triples(self, x: torch.LongTensor, top_k: int = 1):
        ...
        def process(scores, storage, **kwargs):
            if "literal" in kwargs and kwargs["literal"] is not None:
                # For literals, return [(literal_value, 1.0)] style
                storage.append((kwargs['h'], kwargs['r'], [(kwargs['literal'], 1.0)]))
            else:
                # Normal entity prediction
